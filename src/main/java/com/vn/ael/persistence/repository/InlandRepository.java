...
public interface InlandRepository extends GenericRepository<Inland> {
	...
	//Add Phuc 1.8
		@Query("SELECT e FROM Inland e left join fetch e.docsgeneral d left join fetch d.contseals c WHERE (e.docsgeneral.customer.id = :customerId or :customerId is null) and "
		...
		List<Inland> searchInland(@Param("customerId") Long customerId,
				...
				@Param("jobNo") Long jobNo, @Param("contSeal") Long contSeal);
		//End Add Phuc 1.8
}
